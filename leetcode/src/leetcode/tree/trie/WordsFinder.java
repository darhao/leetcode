...
public class WordsFinder {
	...
	private Set<Position> usedSet = new HashSet<>();
    ...
    public List<String> findWords(char[][] board, String[] words) {
    	...
    	for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board[0].length; j++) {
				dfs(new StringBuilder(), trie.root, board, new Position(i, j));
			}
		}
    	//输出结果
    	return result;
    ...
    private void dfs(StringBuilder sb, TrieNode cur, char[][] board, Position position) {
    	if(position.getRow() < 0 || position.getRow() > board.length - 1 || position.getCol() < 0 || position.getCol() > board[0].length - 1) {
    		return;
    	}
    	
    	if(usedSet.contains(position)) {
    		return;
    	}
    	
    	char boardChar = board[position.getRow()][position.getCol()];
    	TrieNode child = cur.children.get(boardChar);
    	...
    	dfs(sb, cur, board, new Position(position.getRow() + 1, position.getCol()));
    	dfs(sb, cur, board, new Position(position.getRow() - 1, position.getCol()));
    	dfs(sb, cur, board, new Position(position.getRow(), position.getCol() + 1));
    	dfs(sb, cur, board, new Position(position.getRow(), position.getCol() - 1));
    	
    	sb.deleteCharAt(sb.length() - 1);
    	usedSet.remove(position);
    }
...
//			new char[] {'i','f','l','v'},
//		};
//		String[] words = new String[] {"oath","oathabc","eat","oathabcdef"};
//		long a = System.currentTimeMillis();
//		for (String string : new WordsFinder().findWords(board, words)) {
//			System.out.println(string);
//		}
//		System.out.println("毫秒：" + (System.currentTimeMillis() - a));
//		
//		
//		board = new char[][] {
//			new char[] {'a','a'},
